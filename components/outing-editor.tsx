...
const SAN_JUAN_WEATHER_LOCATIONS = [
  ...
  { name: 'Rivadavia', latitude: -31.5333, longitude: -68.6 },
  { name: 'San Martín', latitude: -31.5156, longitude: -68.3525 },
  { name: 'Santa Lucía', latitude: -31.5399, longitude: -68.495 },
  { name: 'Sarmiento', latitude: -32.0694, longitude: -68.6917 },
  { name: 'Ullum', latitude: -31.4561, longitude: -68.7 },
  { name: 'Valle Fértil', latitude: -30.6335, longitude: -67.4682 },
  { name: '25 de Mayo', latitude: -31.975, longitude: -68.3639 },
...
export function OutingEditor({ frequentPlaces, conditions, conditionsLoader }: OutingEditorProps) {
  ...
  const loadedConditionsMatchRequest =
    weatherRequestKey !== undefined && loadedConditions?.requestKey === weatherRequestKey
  const displayedConditions = conditions ?? (loadedConditionsMatchRequest ? loadedConditions.value : undefined)
  const conditionsLoading = conditions === undefined && weatherRequestKey !== undefined && !loadedConditionsMatchRequest

  useEffect(() => {
    if (!conditionsLoader || weatherRequestKey === undefined || event.route.estimatedDurationMinutes === undefined) {
      return
    }
  ...
  const estimatedFinish =
    event.trekStart.date && event.trekStart.time && event.route.estimatedDurationMinutes !== undefined
      ? calculateEstimatedFinish(event.trekStart, event.route.estimatedDurationMinutes)
      : undefined
  const daylightMarginMinutes =
    ...
    displayedConditions.sunset.time
      ...
      : undefined
  const daylightStatus = daylightMarginMinutes !== undefined ? getDaylightStatus(daylightMarginMinutes, 60) : undefined
  ...
  const suggestedRecommendations =
    event.route.estimatedDurationMinutes !== undefined
      ? createRecommendations({
          ...
          weather: displayedConditions?.forecastStatus === 'available' ? displayedConditions.weather : undefined,
          daylightStatus,
        })
      : undefined
  ...
  function updateRoute(field: 'distanceKm' | 'elevationGainM', value: string) {
    setEvent((current) => ({
      ...current,
      route: {
        ...current.route,
  ...
  return (
    <section className='mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8'>
      <div className='mb-8'>
        <p className='text-sm font-semibold uppercase tracking-[0.18em] text-emerald-700'>Oxidados Trek</p>
        <h1 className='mt-2 text-3xl font-bold tracking-tight text-slate-950 sm:text-4xl'>
          Generador de salidas de trekking
        </h1>
        <p className='mt-3 max-w-2xl text-sm leading-6 text-slate-600'>
          Completá los datos de la salida y revisá el mensaje antes de compartirlo.
        </p>
      </div>
      ...
      <div className='grid min-w-0 grid-cols-1 gap-6 lg:grid-cols-2 lg:items-start'>
        ...
        >
          <fieldset className='space-y-4'>
            ...
            <label className='block space-y-2 text-sm font-medium text-slate-800'>
              ...
              {validation.fieldErrors.title && (
                <span id='title-error' role='alert' className='text-sm font-medium text-red-700'>
                  {validation.fieldErrors.title}
                </span>
              )}
            </label>
          ...
          <fieldset className='space-y-4'>
            ...
            <label className='block space-y-2 text-sm font-medium text-slate-800'>
              ...
              <input
                ...
                aria-describedby={
                  validation.fieldErrors['meeting.location.placeName'] ? 'meeting-place-error' : undefined
                }
                onChange={(e) => updateMeeting('placeName', e.target.value)}
              />
              {validation.fieldErrors['meeting.location.placeName'] && (
                <span id='meeting-place-error' role='alert' className='text-sm font-medium text-red-700'>
                  {validation.fieldErrors['meeting.location.placeName']}
                </span>
              )}
            </label>
          ...
          <fieldset className='space-y-4'>
            ...
            <label className='block space-y-2 text-sm font-medium text-slate-800'>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  const location = SAN_JUAN_WEATHER_LOCATIONS.find((candidate) => candidate.name === e.target.value)
            ...
            <div role='status' aria-live='polite' className='space-y-2 text-sm text-slate-600'>
              {conditionsLoading && <p>Cargando pronóstico…</p>}
              {displayedConditions?.forecastStatus === 'available' && (
                <div className='space-y-2 text-sm text-slate-600'>
                  ...
                  {displayedConditions.sunrise.time && <p>Amanecer: {displayedConditions.sunrise.time}</p>}
                  {displayedConditions.sunset.time && <p>Atardecer: {displayedConditions.sunset.time}</p>}
                  {estimatedFinish && <p>Fin estimado: {estimatedFinish.time}</p>}
                  {daylightMarginMinutes !== undefined && <p>Margen de luz: {daylightMarginMinutes} min</p>}
                  {daylightStatus === 'approachingSunset' && <p>Atención: la salida termina cerca del atardecer</p>}
                  {daylightStatus === 'afterSunset' && <p>Atención: la salida termina después del atardecer</p>}
                </div>
              )}
              ...
              {displayedConditions?.forecastStatus === 'error' && <p>No se pudo consultar el pronóstico</p>}
            </div>
          </fieldset>
          ...
          <fieldset className='space-y-4'>
            ...
            {placeError && (
              <p id='place-error' role='alert' className='text-sm font-medium text-red-700'>
                {placeError}
              </p>
            )}
            <label className='block space-y-2 text-sm font-medium text-slate-800'>
            ...
            <label className='block space-y-2 text-sm font-medium text-slate-800'>
              ...
              {validation.fieldErrors['route.distanceKm'] && (
                <span id='distance-error' role='alert' className='text-sm font-medium text-red-700'>
                  {validation.fieldErrors['route.distanceKm']}
                </span>
              )}
            </label>
            ...
            <label className='block space-y-2 text-sm font-medium text-slate-800'>
              ...
              {validation.fieldErrors['route.difficulty'] && (
                <span id='difficulty-error' role='alert' className='text-sm font-medium text-red-700'>
                  {validation.fieldErrors['route.difficulty']}
                </span>
              )}
            </label>
          ...
          <fieldset className='space-y-4'>
            ...
            {recommendations ? (
              <>
                <label className='block space-y-2 text-sm font-medium text-slate-800'>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setManualHydrationLiters(e.target.value === '' ? undefined : Number(e.target.value))
                    }
                  />
                </label>
                {recommendations.hydration && (
                  <p className='text-sm text-slate-600'>Orientativo: {recommendations.hydration.reasons.join('. ')}</p>
                )}
                <button
              ...
              <p className='text-sm text-slate-600'>Completá la duración para obtener recomendaciones.</p>
            )}
          </fieldset>
          ...
          <fieldset className='space-y-4'>
            ...
            {validation.responsibleError && (
              <p id='responsible-error' role='alert' className='text-sm font-medium text-red-700'>
                {validation.responsibleError}
              </p>
            )}
          </fieldset>
          ...
          <fieldset className='space-y-4'>
              ...
              <label key={requirement} className='flex items-start gap-3'>
                <input
                  ...
                  onChange={(e) =>
                    setEvent((current) => ({
                      ...current,
                      requirements: toggleRequirement(current.requirements, requirement, e.target.checked),
                    }))
                  }
                />
                {requirement}
            ...
            {event.requirements
              .filter(
                (requirement) => !DEFAULT_REQUIREMENTS.includes(requirement as (typeof DEFAULT_REQUIREMENTS)[number]),
              )
              .map((requirement) => (
                <label key={requirement} className='flex items-start gap-3'>
                  <input
            ...
            <button
              className='rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-600 focus-visible:ring-offset-2'
              type='button'
        ...
        <div className='min-w-0 lg:sticky lg:top-6'>
          <MessagePreview event={event} recommendations={recommendations} conditions={displayedConditions} />
        </div>
      </div>
    </section>
  )
